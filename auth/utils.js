...
// this objects contains information that will be passed as query params to the auth // token endpoint
  const auth_token_params = {query_params,
    response_type: 'code',
  };
...
// a url formed with the auth token endpoint and the
  const request_get_auth_code_url = `${google_auth_token_endpoint}?${querystring.stringify(auth_token_params)}&scope=${scopes.join (' ')}`;
